...
// =============================================================================
// DEMO 4b — sh_transformsGL  (§ 2D Transforms)
// Translate/scale/rotate the SPACE (uv); a 2x2 rotation matrix is the trick.
// =============================================================================
DEMO_HTML.sh_transformsGL = {
    title: 'Shaders — 2D Transforms',
    canvas: { width: 800, height: 450 },
    controls: [
        { id: 'btnXfTranslate', text: 'Translate' },
        { id: 'btnXfScale',     text: 'Scale' },
        { id: 'btnXfRotate',    text: 'Rotate' },
        { id: 'btnXfAll',       text: 'All composed' }
    ],
    info: 'You transform the coordinate, not the shape. Try each button.'
};

DEMO_CODE.sh_transformsGL = `(function transformsShader() {
    const canvas = document.getElementById('canvas');
    const info = document.getElementById('info');

    // One transform line is injected so buttons swap behaviour with no
    // WebGL plumbing — same state-built pattern as the water mini-project.
...
DEMO_CODE_TS.sh_transformsGL = `(function transformsShader(): void {
    const canvas = document.getElementById('canvas') as HTMLCanvasElement;
    const info = document.getElementById('info') as HTMLDivElement;

    function buildFrag(xform: string): string {
        return \`precision mediump float;
uniform vec2 u_resolution;
uniform float u_time;
mat2 rot(float a) { float s = sin(a), c = cos(a); return mat2(c, -s, s, c); }
...
void main() {
  ...
  gl_FragColor = vec4(mix(vec3(0.05, 0.07, 0.15), vec3(0.31, 0.76, 0.97), m), 1.0);
    ...
    const toy = makeShaderToy(canvas, buildFrag(MODES.rotate), { info: info });
    function set(mode: string, msg: string): void { toy.setFrag(buildFrag(MODES[mode])); info.textContent = msg; }

    document.getElementById('btnXfTranslate')?.addEventListener('click', (): void =>
        set('translate', 'Translate: p -= offset re-centers the space; the box behaves as if at offset.'));
    document.getElementById('btnXfScale')?.addEventListener('click', (): void =>
        set('scale', 'Scale: dividing p zooms — a smaller divisor makes a bigger shape.'));
    document.getElementById('btnXfRotate')?.addEventListener('click', (): void =>
        set('rotate', 'Rotate: a 2x2 rotation matrix spins the whole plane around the origin.'));
    document.getElementById('btnXfAll')?.addEventListener('click', (): void =>
        set('all', 'All three composed — order matters: rotate, then scale, then translate.'));
})();`;

// =============================================================================
// DEMO 4c — sh_polarGL  (§ Polar Coordinates)
// r = length(uv), a = atan(uv.y, uv.x) → radial gradients, cooldown UI, spirals.
// =============================================================================
DEMO_HTML.sh_polarGL = {
    title: 'Shaders — Polar Coordinates',
    canvas: { width: 800, height: 450 },
    controls: [
        { id: 'btnPolarRadius',   text: 'Radius' },
        { id: 'btnPolarAngle',    text: 'Angle' },
        { id: 'btnPolarCooldown', text: 'Cooldown swipe' },
        { id: 'btnPolarSpiral',   text: 'Spiral' }
    ],
    info: 'Radius + angle unlock every "round" effect in 2D games.'
};

DEMO_CODE.sh_polarGL = `(function polarShader() {
    const canvas = document.getElementById('canvas');
    const info = document.getElementById('info');
...
\`;
  ...
  gl_FragColor = vec4(col, 1.0);
    ...
    const COOLDOWN = HEAD + \`  float ang = atan(uv.x, -uv.y) / TAU + 0.5;
...
DEMO_CODE_TS.sh_polarGL = `(function polarShader(): void {
    const canvas = document.getElementById('canvas') as HTMLCanvasElement;
    const info = document.getElementById('info') as HTMLDivElement;

    const HEAD: string = \`precision mediump float;
uniform vec2 u_resolution;
uniform float u_time;
#define TAU 6.2831853
void main() {
...
\`;
    ...
    const RADIUS: string = HEAD + \`  vec3 col = mix(vec3(0.31, 0.76, 0.97), vec3(0.04, 0.06, 0.12), clamp(r, 0.0, 1.0));
  gl_FragColor = vec4(col, 1.0);
}\`;

    const ANGLE: string = HEAD + \`  float seg = step(0.5, fract((a / TAU + 0.5) * 6.0));
  vec3 col = mix(vec3(0.10, 0.20, 0.35), vec3(1.0, 0.65, 0.30), seg);
  gl_FragColor = vec4(col, 1.0);
}\`;

    const COOLDOWN: string = HEAD + \`  float ang = atan(uv.x, -uv.y) / TAU + 0.5;
  float fill = fract(u_time * 0.25);
  float ring = step(r, 0.72) * (1.0 - step(r, 0.48));
  float ready = step(ang, fill);
  vec3 col = mix(vec3(0.10, 0.12, 0.20), vec3(0.31, 0.76, 0.97),
  ...
  gl_FragColor = vec4(col, 1.0);
    ...
    const SPIRAL: string = HEAD + \`  float s = 0.5 + 0.5 * sin(a * 5.0 + r * 26.0 - u_time * 3.0);
  vec3 col = mix(vec3(0.04, 0.06, 0.12), vec3(0.40, 0.85, 0.70), s * (1.0 - r));
  gl_FragColor = vec4(col, 1.0);
}\`;

    const toy = makeShaderToy(canvas, RADIUS, { info: info });
    document.getElementById('btnPolarRadius')?.addEventListener('click', (): void => {
        toy.setFrag(RADIUS); info.textContent = 'r = length(uv) → a radial gradient (glow / vignette).';
    });
    document.getElementById('btnPolarAngle')?.addEventListener('click', (): void => {
        toy.setFrag(ANGLE); info.textContent = 'a = atan(uv.y, uv.x) → angular slices (pie wedges).';
    });
    document.getElementById('btnPolarCooldown')?.addEventListener('click', (): void => {
        toy.setFrag(COOLDOWN); info.textContent = 'Angle + radius → an ability-cooldown swipe (real game UI).';
    });
    document.getElementById('btnPolarSpiral')?.addEventListener('click', (): void => {
        toy.setFrag(SPIRAL); info.textContent = 'sin(angle * k + radius * k - time) → a spiral.';
    });
})();`;

// =============================================================================
// DEMO 4d — sh_sdfCombineGL  (§ Combining Shapes with SDFs)
// min=union, max=intersect, max(a,-b)=subtract, smin=smooth blend (metaballs).
// =============================================================================
DEMO_HTML.sh_sdfCombineGL = {
    title: 'Shaders — Combining Shapes (SDFs)',
    canvas: { width: 800, height: 450 },
    controls: [
        { id: 'btnSdfUnion',     text: 'Union' },
        { id: 'btnSdfIntersect', text: 'Intersect' },
        { id: 'btnSdfSubtract',  text: 'Subtract' },
        { id: 'btnSdfSmooth',    text: 'Smooth blend' }
    ],
    info: 'The circle slides through the box — watch each boolean op.'
};

DEMO_CODE.sh_sdfCombineGL = `(function sdfCombineShader() {
    const canvas = document.getElementById('canvas');
    const info = document.getElementById('info');
...
DEMO_CODE_TS.sh_sdfCombineGL = `(function sdfCombineShader(): void {
    const canvas = document.getElementById('canvas') as HTMLCanvasElement;
    const info = document.getElementById('info') as HTMLDivElement;

    function buildFrag(op: string): string {
        return \`precision mediump float;
uniform vec2 u_resolution;
uniform float u_time;
float sdCircle(vec2 p, float r) { return length(p) - r; }
...
void main() {
  ...
  gl_FragColor = vec4(col, 1.0);
    ...
    const toy = makeShaderToy(canvas, buildFrag(OPS.union), { info: info });
    function set(op: string, msg: string): void { toy.setFrag(buildFrag(OPS[op])); info.textContent = msg; }

    document.getElementById('btnSdfUnion')?.addEventListener('click', (): void =>
        set('union', 'min(a, b) → UNION: both shapes, whichever edge is closer.'));
    document.getElementById('btnSdfIntersect')?.addEventListener('click', (): void =>
        set('intersect', 'max(a, b) → INTERSECTION: only where the two overlap.'));
    document.getElementById('btnSdfSubtract')?.addEventListener('click', (): void =>
        set('subtract', 'max(a, -b) → SUBTRACTION: the box carves the circle.'));
    document.getElementById('btnSdfSmooth')?.addEventListener('click', (): void =>
        set('smooth', 'smin(a, b, k) → SMOOTH union: a liquid / metaball blend.'));
})();`;
