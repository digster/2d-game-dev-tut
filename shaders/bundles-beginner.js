...
//
// ── GLSL-in-a-template-literal convention ────────────────────────────────────
// Every DEMO_CODE.sh_* below is itself a JS template literal. The fragment
// shaders inside are written as an inner template literal whose backticks are
// escaped (\`). GLSL here deliberately contains NO backtick and NO `${` so the
// ONLY escaping needed is the inner delimiters. Any JS string interpolation in
// a demo body (e.g. info text) is escaped as \${...}. Verify exports by
// actually running them, never by eye — double-nesting is the #1 footgun here.
// =============================================================================
...
DEMO_CODE_TS.sh_uvGL = `(function uvShader(): void {
    ...
    const CENTERED: string = HEAD +
        'void main() {\\n' +
        '  vec2 uv = gl_FragCoord.xy / u_resolution * 2.0 - 1.0;\\n' +
        '  gl_FragColor = vec4(uv * 0.5 + 0.5, 0.0, 1.0);\\n' +
        '}';

    const ASPECT: string = HEAD +
        'void main() {\\n' +
        '  vec2 uv = (gl_FragCoord.xy / u_resolution) * 2.0 - 1.0;\\n' +
        '  uv.x *= u_resolution.x / u_resolution.y;\\n' +
        '  float d = step(length(uv), 0.6);\\n' +
...
DEMO_CODE_TS.sh_colorGL = `(function colorShader(): void {
    ...
    const LINEAR: string = HEAD +
        'void main() {\\n' +
        '  float t = gl_FragCoord.x / u_resolution.x;\\n' +
        '  gl_FragColor = vec4(mix(A, B, t), 1.0);\\n' +
        '}';

    const RADIAL: string = HEAD +
        'void main() {\\n' +
        '  vec2 uv = gl_FragCoord.xy / u_resolution;\\n' +
        '  uv.x *= u_resolution.x / u_resolution.y;\\n' +
        '  float c = u_resolution.x / u_resolution.y * 0.5;\\n' +
