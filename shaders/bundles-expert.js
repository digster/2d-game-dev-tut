...
// =============================================================================
// DEMO 7 — sh_hatchGL  (§ Cross-Hatch & Halftone Shading)
// =============================================================================
DEMO_HTML.sh_hatchGL = {
    title: 'Shaders — Cross-Hatch & Halftone Shading',
    canvas: { width: 800, height: 450 },
    controls: [
        { id: 'btnHatchHatch',  text: 'Cross-hatch' },
        { id: 'btnHatchHalf',   text: 'Halftone' },
        { id: 'btnHatchDither', text: 'Dither' },
        { id: 'btnHatchCoarse', text: 'Coarser' }
    ],
    info: 'Throw away colour, keep luminance, redraw it as ink.'
};

DEMO_CODE.sh_hatchGL = `(function hatchShader() {
    const canvas = document.getElementById('canvas');
    const info = document.getElementById('info');
...
float ink(vec2 px, float L){
  ...
  float r = sqrt(1.0 - clamp(L, 0.0, 1.0)) * 0.62;
...
float ink(vec2 px, float L){ return step(ign(px), L); }\`
    };
...
void main() {
  ...
  gl_FragColor = vec4(mix(backdrop(uv), styl, tex.a), 1.0);
    ...
    const toy = makeShaderToy(canvas, buildFrag(state.mode), { info: info, sprite: drawSprite, param: 0.4 });
...
DEMO_CODE_TS.sh_hatchGL = `(function hatchShader(): void {
    const canvas = document.getElementById('canvas') as HTMLCanvasElement;
    const info = document.getElementById('info') as HTMLDivElement;

    type HatchMode = 'hatch' | 'halftone' | 'dither';
    const INK: Record<HatchMode, string> = {
        hatch: \`float ink(vec2 px, float L){
  float F = mix(0.06, 0.16, u_param);
  float k = 1.0;
  if (L < 0.85) k = min(k, step(0.5, fract((px.x + px.y) * F)));
...
float ink(vec2 px, float L){
  ...
  return smoothstep(r - 0.06, r + 0.06, length(cell));
        ...
        dither: \`float ign(vec2 p){ return fract(52.9829189 * fract(dot(p, vec2(0.06711056, 0.00583715)))); }
float ink(vec2 px, float L){ return step(ign(px), L); }\`
    } as Record<HatchMode, string>;

    function buildFrag(mode: HatchMode): string {
        return GLSL_HEAD + \`\${INK[mode]}
void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution;
  vec4 tex = texture2D(u_tex, toSpriteUV(uv));
  ...
  gl_FragColor = vec4(mix(backdrop(uv), styl, tex.a), 1.0);
    ...
    const state = { mode: 'hatch' as HatchMode };
    const toy = makeShaderToy(canvas, buildFrag(state.mode), { info: info, sprite: drawSprite, param: 0.4 });
    function setMode(m: HatchMode, msg: string): void { state.mode = m; toy.setFrag(buildFrag(m)); info.textContent = msg; }

    document.getElementById('btnHatchHatch')?.addEventListener('click', (): void =>
        setMode('hatch', 'Cross-hatch: darker luminance unlocks more rotated line layers.'));
    document.getElementById('btnHatchHalf')?.addEventListener('click', (): void =>
        setMode('halftone', 'Halftone: a rotated dot grid; dot radius grows as it darkens.'));
    document.getElementById('btnHatchDither')?.addEventListener('click', (): void =>
        setMode('dither', 'Dither: interleaved-gradient-noise threshold → 1-bit ink.'));
    document.getElementById('btnHatchCoarse')?.addEventListener('click', (): void => {
        toy.setParam(0.95); info.textContent = 'Coarser screen — u_param scales the line/dot frequency.';
    });
})();`;

// =============================================================================
// DEMO 8 — sh_stylizeGL  (§ Stylized Sprite: ASCII & Oil Paint)
// =============================================================================
DEMO_HTML.sh_stylizeGL = {
    title: 'Shaders — Stylized Sprite: ASCII & Oil Paint',
    canvas: { width: 800, height: 450 },
    controls: [
        { id: 'btnStyNormal', text: 'Normal' },
        { id: 'btnStyAscii',  text: 'ASCII' },
        { id: 'btnStyOil',    text: 'Oil paint' },
        { id: 'btnStyCell',   text: 'Detail +' }
    ],
    info: 'ASCII collapses cells to a glyph; Kuwahara keeps the flattest quadrant.'
};

DEMO_CODE.sh_stylizeGL = `(function stylizeShader() {
    const canvas = document.getElementById('canvas');
    const info = document.getElementById('info');
  ...
  vec4 cellTex = texture2D(u_tex, (id + 0.5) / CS);
...
void main() {
  ...
  gl_FragColor = vec4(mix(backdrop(uv), col, a), 1.0);
    ...
    const toy = makeShaderToy(canvas, buildFrag(state.mode, state.cs, state.k), { info: info, sprite: drawSprite });
...
DEMO_CODE_TS.sh_stylizeGL = `(function stylizeShader(): void {
    const canvas = document.getElementById('canvas') as HTMLCanvasElement;
    const info = document.getElementById('info') as HTMLDivElement;

    type StyMode = 'normal' | 'ascii' | 'oil';
    function buildFrag(mode: StyMode, cs: number, k: number): string {
        const BODY: Record<StyMode, string> = {
            normal: \`void main() {
  vec2 uv = gl_FragCoord.xy / u_resolution;
  vec4 tex = texture2D(u_tex, toSpriteUV(uv));
  gl_FragColor = vec4(mix(backdrop(uv), tex.rgb, tex.a), 1.0);
...
void main() {
  ...
  gl_FragColor = vec4(mix(backdrop(uv), col, a), 1.0);
        ...
        } as Record<StyMode, string>;
        return GLSL_HEAD + BODY[mode];
    }

    const state = { mode: 'ascii' as StyMode, cs: 30, k: 2 };
    const toy = makeShaderToy(canvas, buildFrag(state.mode, state.cs, state.k), { info: info, sprite: drawSprite });
    function refresh(msg: string): void { toy.setFrag(buildFrag(state.mode, state.cs, state.k)); info.textContent = msg; }

    document.getElementById('btnStyNormal')?.addEventListener('click', (): void => {
        state.mode = 'normal'; refresh('Baseline — the raw faceted sprite, no stylization.');
    });
    document.getElementById('btnStyAscii')?.addEventListener('click', (): void => {
        state.mode = 'ascii'; refresh('ASCII: one luminance sample per cell → brightness-keyed strokes.');
    });
    document.getElementById('btnStyOil')?.addEventListener('click', (): void => {
        state.mode = 'oil'; refresh('Oil: Kuwahara — the lowest-variance quadrant mean flattens detail.');
    });
    document.getElementById('btnStyCell')?.addEventListener('click', (): void => {
        state.cs = state.cs >= 44 ? 18 : state.cs + 8;
        state.k = state.k >= 3 ? 1 : state.k + 1;
        refresh('Detail ' + state.cs + ' / kernel ' + state.k + ' — both are shader constants → rebuild.');
    });
