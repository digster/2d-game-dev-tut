// =============================================================================
// ISOMETRIC STRATEGY — ADVANCED TIER DEMOS
// =============================================================================
// 8 demos. Each one is wrapped in an IIFE; they share a small set of helpers
// (palette, terrain renderer, MinHeap, flow-field BFS, drawIsoUnit) defined at
// the top. The stress test uses the MinHeap to keep frame-rate stable.
// =============================================================================
...
function buildOpenMap(w = 16, h = 12) {
    const tiles = Array.from({ length: h }, () => Array(w).fill('grass'));
    return { width: w, height: h, tiles };
}

function buildRiverMap(w = 16, h = 12) {
    const tiles = Array.from({ length: h }, () => Array(w).fill('grass'));
...
function drawIsoUnit(ctx, cx, cy, tW, tH, ox, oy, color, hp = null, maxHp = null, selected = false) {
...
function drawIsoBuilding(ctx, anchorX, anchorY, fw, fh, tW, tH, ox, oy, color = AV_COLORS.bldgFill) {
...
// Reusable binary heap for A*. Used by stress test and multi-tile pathing demos.
class MinHeap {
...
// Heap-backed A*. Accepts a blocked Uint8Array (0=open, 1=blocked) so callers
// can fold building footprints into terrain in one place.
function aStarHeap(map, blocked, start, goal) {
...
// BFS flow field: cost grid + 8-vector flow per cell.
function computeFlowField(map, blocked, goal) {
    const W = map.width, H = map.height;
...
// =============================================================================
(function fogDemo() {
    const canvas = document.getElementById('fogDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('fogDemoInfo');
    ...
    const fog = new Uint8Array(map.width * map.height); // start hidden
    let dragging = false;
    let showFog = true;
    ...
    function render() {
        ...
        // Draw enemies only on visible cells
...
// =============================================================================
(function flowDemo() {
    const canvas = document.getElementById('flowDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('flowDemoInfo');
        ...
        if (showCost) {
            ...
            for (let cy = 0; cy < map.height; cy++) {
                for (let cx = 0; cx < map.width; cx++) {
                    ...
                    const t = 1 - c / maxCost; // brighter near goal
                    const p = cartToIso(cx, cy, tW, tH, ox, oy);
                    drawIsoTile(ctx, p.x, p.y, tW, tH,
        ...
        if (showArrows) {
            ...
            for (let cy = 0; cy < map.height; cy++) {
                for (let cx = 0; cx < map.width; cx++) {
                    ...
                    const ey = p.y + f.dy * len * 0.6; // squish y so arrows look right in iso
                    ctx.beginPath();
                    ctx.moveTo(p.x, p.y);
                    ctx.lineTo(ex, ey);
                    ctx.stroke();
...
// =============================================================================
// DEMO 3 — multiTileDemo (A* around 2x2 buildings)
// =============================================================================
(function multiTileDemo() {
    const canvas = document.getElementById('multiTileDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('multiTileDemoInfo');
...
// =============================================================================
// DEMO 4 — spatialHashDemo
// =============================================================================
(function spatialHashDemo() {
    const canvas = document.getElementById('spatialHashDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('spatialHashDemoInfo');
    ...
    function render() {
        ...
        // Units as small dots (300 of them; drawUnitGlyph too heavy here)
        for (const u of UNITS) {
    ...
    canvas.addEventListener('mousemove', (e) => {
        const m = getMouseLocal(canvas, e);
        // Convert mouse → fractional tile (for the centred query)
...
(function saveLoadDemo() {
    const canvas = document.getElementById('saveLoadDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('saveLoadDemoInfo');
    ...
    function save() {
        ...
        out.innerHTML = `<code class="language-javascript">${savedJson.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</code>`;
...
// =============================================================================
// DEMO 6 — mapEditorDemo
// =============================================================================
(function mapEditorDemo() {
    const canvas = document.getElementById('mapEditorDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('mapEditorDemoInfo');
    ...
    const buildings = [];
    let brush = 'grass'; // 'grass'|'water'|'sand'|'stone'|'house'|'erase'
    let painting = false;
    ...
    function paintAt(mx, my) {
        ...
        if (!t) return;
        if (brush === 'house') {
            // Place a 2×2 only on valid footprint
        ...
        if (brush === 'erase') {
            // Remove any building anchored at exactly this tile, otherwise set terrain to grass
    ...
    document.getElementById('btnEdExport')?.addEventListener('click', () => {
        ...
        out.innerHTML = `<code class="language-javascript">${json.replace(/&/g, '&amp;').replace(/</g, '&lt;')}</code>`;
...
// =============================================================================
// DEMO 7 — aiDemo (two AI opponents play each other)
// =============================================================================
(function aiDemo() {
    const canvas = document.getElementById('aiDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('aiDemoInfo');
    ...
    function tickAI(ai, team, dt) {
        ...
        ai.timer = 1.0;
        ai.resources += 5; // baseline economy (workers contribute more)
...
// =============================================================================
// DEMO 8 — stressDemo (500 units, flow-field follow, FPS readout)
// =============================================================================
(function stressDemo() {
    const canvas = document.getElementById('stressDemo');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    const info = document.getElementById('stressDemoInfo');
    ...
    function render() {
        ...
        const gp = cartToIso(goal.x, goal.y, tW, tH, ox, oy);
        drawIsoTile(ctx, gp.x, gp.y, tW, tH, 'rgba(255, 167, 38, 0.5)', AV_COLORS.accent);
        // Units (tiny dots for performance)
