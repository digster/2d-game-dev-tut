...
// ---- Two reference configs: the Beginner ("raw") feel, and the full "juiced" feel.
// Every field is a knob the demos flip. Times are in milliseconds; gravities and
// speeds are px/s (and px/s²). RAW reproduces the Beginner tier exactly.
const RAW_CFG = {
...
// =============================================================================
// PlayerBody — the whole feel kit behind config flags.
// Set 0 / false on a flag and that feature is simply absent (Beginner behaviour).
// =============================================================================
class PlayerBody {
    constructor(map, x, y, cfg) {
        ...
        this.state = 'idle';     // idle | run | jump | fall | land
        this.landTimer = 0;
        // per-step telemetry the demos visualise
        this.justJumped = false;
        this.usedCoyote = false;
    ...
    update(dt, input) {
        ...
        // ---- horizontal: accelerate while held, friction when released ----
        ...
        if (pressedJump) this.bufferLeft = c.bufferMs > 0 ? c.bufferMs / 1000 : 1e-4; // tiny = "this frame only"
        ...
        if (this.bufferLeft > 0 && canJump) {
            this.usedBuffer = pressedJump ? false : (c.bufferMs > 0); // fired from a stored press, not this frame's
            this.usedCoyote = !this.onGround;                          // jumped while airborne = coyote saved us
            this.vy = -c.jumpSpeed;
            this.jumping = true;
            this.justJumped = true;
            this.squash = 1.35;          // stretch on takeoff
            this.bufferLeft = 0;
            this.coyoteLeft = 0;
        }

        // ---- variable height: releasing the button while rising cuts the jump ----
        const holdingJump = input.isDown('jump') || input.isDown('up');
        if (c.variable && this.jumping && !holdingJump && this.vy < 0) {
            this.vy *= c.cutFactor;
            this.jumping = false;
        }
        if (this.vy >= 0) this.jumping = false;

        // ---- gravity: asymmetric (rise vs fall), apex hang, fast-fall ----
        let g = this.vy < 0 ? c.gravJump : c.gravFall;
        if (c.apexThreshold > 0 && Math.abs(this.vy) < c.apexThreshold) g *= c.apexMult; // lighter near the top
        ...
        const hx = moveAndCollide(this.box, this.vx * dt, 0, this.map);
        if (hx.left || hx.right) this.vx = 0;

        const beforeY = this.box.y;
        const hy = moveAndCollide(this.box, 0, this.vy * dt, this.map);

        // corner correction: a head-bonk while rising is forgiven if a small
        // sideways nudge would have cleared the ledge corner.
        if (hy.up && this.vy < 0 && c.corner > 0) {
            ...
            for (let mag = 1; mag <= c.corner && !fixed; mag++) {
                for (const dir of [this.facing, -this.facing]) {
                    ...
                    const th = moveAndCollide(test, 0, this.vy * dt, this.map);
                    if (!th.up) { this.box.x = test.x; this.box.y = test.y; this.cornerNudge = dir * mag; fixed = true; break; }
                }
            }
            if (!fixed) this.vy = 0; // a real ceiling, not a corner
        ...
        this.squash += (1 - this.squash) * Math.min(1, dt * 14); // relax toward neutral

        // ---- finite state machine ----
        if (this.landTimer > 0) { this.landTimer -= dt; this.state = 'land'; }
        else if (!this.onGround) this.state = this.vy < 0 ? 'jump' : 'fall';
        else this.state = Math.abs(this.vx) > 12 ? 'run' : 'idle';
    }
}

// ---- shared draw helpers (re-declared per tier; pf-prefixed, no collisions) --
const PF_STATE_COLOR = { idle: '#7CF2C8', run: '#7CF2C8', jump: '#66bb6a', fall: '#ffa726', land: '#4fc3f7' };

function pfDrawBody(ctx, b, colorOverride) {
    const sq = clamp(b.squash, 0.5, 1.5);
    const w = b.box.w / sq;            // squashed = wider
    const h = b.box.h * sq;            // squashed = shorter
