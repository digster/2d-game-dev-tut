...
//
// THE NAVIGATION ALGORITHMS are TOP-LEVEL here on purpose — they're this tier's
// lesson, and being top-level means you can call them from the DevTools console
// (`tdAStar(...)`, `tdFlowField(...)`, `tdLineOfSight(...)`). They are NOT on
// `window` yet: that happens when the Expert (scale) tier becomes their 2nd
// consumer and they're promoted to `engine/nav.js`.
//
// DEPENDENCIES: ../shared/utils.js, engine/loop.js, engine/render.js,
//   engine/world.js, engine/entities.js (the capstone reuses TDTower/TDProjectile).
// =============================================================================
...
// is the upgrade for large ones, exactly the kind of thing the Expert tier cares about.)
...
// occupy the cell, run A* spawn→goal, restore, and report whether it sealed the
// maze. This is what stops a maze-TD from letting you wall the goal off entirely.
...
// to the goal — that's the "integration field". Then each cell stores a unit
// vector toward its lowest-cost 8-neighbour — the "flow field". A creep anywhere
// just reads its cell's vector and walks: one field, any number of creeps, O(1)
// each. (Uniform step cost ⇒ a plain BFS computes the exact distances.)
...
// (the endpoints are skipped — the shooter and target tiles don't block their own
// shot). This is the raycast that makes a wall a real piece of cover.
